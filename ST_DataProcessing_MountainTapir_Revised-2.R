...
#read in data
mt_dat<- read.csv("Data Processing/all_data_Mountain4.csv" )
head(mt_dat)
dim(mt_dat)
length(sort(unique(mt_dat$Camr_Nm)))	# Camr_Nm GOES FROM 1 TO 87, THERE IS NO CT 33 AND NO 44!

s <- read.csv("Data Processing/Data Formatting - Global Tapir Project_JLM_stations.csv")
head(s)
tail(s)
dim(s)	
length(sort(unique(mt_dat$Camr_Nm)))	# OK, IT MATCHES WITH mt_dat!

 #fix error in date format ##Dont need to do this if dates are fine
 s$Cmr_E_D[34] <- "08/29/2016"
 mt_dat$Cmr_E_D[57] <- "08/29/2016"
 mt_dat$Cmr_E_D[58] <- "08/29/2016"

#unique stations - 85 (28 with records) --> enter in how many stations costa rica has
...
recs_loc <- dat1[!is.na(dat1$Date),c("Camr_Nm", "Species", "Date", "Time", "Latitud", "Longitd")]	# Excluding ROW THAT ARE NOT TAPIR RECORDS, BUT WERE ADDED BY SARAH
dim(recs_loc)
